...
type FeedRequest struct {
	LatestTime *int64  `json:"latest_time" form:"latest_time" query:"latest_time"` // 可选参数，限制返回视频的最新投稿时间戳，精确到秒，不填表示当前时间
	Token      *string `json:"token" form:"token" query:"token"`                   // 可选参数，登录用户设置
}
...
type FeedResponse struct {
	...
	NextTime   *int64   `json:"next_time" form:"next_time" query:"next_time"`       // 本次返回的视频中，发布最早的时间，作为下次请求时的latest_time
}

type Video struct {
	Id            int64      `json:"id" form:"id" query:"id"`                                     // 视频唯一标识
	Author        *auth.User `json:"author" form:"author" query:"author"`                         // 视频作者信息
	PlayUrl       string     `json:"play_url" form:"play_url" query:"play_url"`                   // 视频播放地址
	CoverUrl      string     `json:"cover_url" form:"cover_url" query:"cover_url"`                // 视频封面地址
